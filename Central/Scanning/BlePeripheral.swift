...
class BlePeripheral: NSObject, CBPeripheralDelegate {
    ...
    func writeValue(value: String, to characteristic: CBCharacteristic) {
        /*
        let intValue = 8;
        let intByteValue = Data(intvalue)
        
        let floatValue = 10.2
        let floatByteValue1 = Data(floatValue)
        */
        /*
        var intValue = 8;
        let intDataValue = Data(buffer: UnsafeBufferPointer(start: &intValue, count: 1))
        
        var floatValue = 10.2;
        let floatDataValue = Data(buffer: UnsafeBufferPointer(start: &floatValue, count: 1))
        */
